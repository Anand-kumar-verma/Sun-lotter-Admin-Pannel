...
const ColorPrediction1Min = () => {
  ...
  React.useEffect(() => {
    const handleOneMin = (onemin) => {
      ...
      if (onemin === 0) client.refetchQueries("get_next_id_one_min");
    };
  ...
  async function manuallyWinningAPI(reqBody) {
    ...
    try {
      const res = await axiosInstance.post(API_URLS.manually_winning_api, newreqBody);
  ...
  async function manuallyUpdatePercentage() {
    ...
    try {
      const res = await axiosInstance.post(API_URLS.manually_update_percentage_api, newreqBody);
      toast(res?.data?.message);
    } catch (e) {
      console.log(e);
    }
